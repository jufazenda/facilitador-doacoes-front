    ...
    <header className="sticky top-0 z-50 border-b border-purple-100 bg-white/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:h-20 sm:px-6">
        <Link
          ...
          className="flex items-center gap-2 text-xl font-extrabold text-purple-800 sm:gap-3 sm:text-2xl"
        >
          <span className="flex h-9 w-9 -rotate-6 items-center justify-center rounded-xl border-2 border-purple-700 text-lg text-accent sm:h-10 sm:w-10 sm:rounded-2xl">
            ♡
          </span>
          Elo Solidário
        </Link>
        ...
        <nav className="hidden items-center gap-10 font-semibold text-purple-950 md:flex">
          <a href="/#campaigns" className="transition hover:text-purple-600">Campanhas</a>
          <a href="/#institutions" className="transition hover:text-purple-600">Instituições</a>
          <a href="/#how-it-works" className="transition hover:text-purple-600">Como funciona?</a>
        </nav>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {user ? (
            ...
            <Link to="/login" className="hidden font-semibold text-purple-950 sm:block hover:text-purple-600 transition">
              Login
            </Link>
          )}
      ...
      {menuOpen && (
        <div className="border-t border-purple-100 bg-white px-4 pb-6 pt-4 md:hidden">
          <nav className="flex flex-col gap-1">
            <a href="/#campaigns" onClick={() => setMenuOpen(false)} className="rounded-xl px-4 py-3 font-semibold text-purple-950 hover:bg-purple-50">Campanhas</a>
            <a href="/#institutions" onClick={() => setMenuOpen(false)} className="rounded-xl px-4 py-3 font-semibold text-purple-950 hover:bg-purple-50">Instituições</a>
            <a href="/#how-it-works" onClick={() => setMenuOpen(false)} className="rounded-xl px-4 py-3 font-semibold text-purple-950 hover:bg-purple-50">Como funciona?</a>
          </nav>
          ...
          <div className="mt-4 border-t border-purple-100 pt-4 flex flex-col gap-3">
            {user ? (
              <>
                ...
                <Link to={`/area/${user.tipo}`} onClick={() => setMenuOpen(false)} className="rounded-xl bg-purple-50 px-4 py-3 text-center font-semibold text-purple-800">
                  Minha área
                </Link>
                <button onClick={handleLogout} className="rounded-xl border border-red-100 px-4 py-3 text-center font-semibold text-red-500">
                  Sair
                </button>
              </>
            ) : (
              <>
                <Link to="/login" onClick={() => setMenuOpen(false)} className="rounded-xl border border-purple-200 px-4 py-3 text-center font-semibold text-purple-800">
                  Login
                </Link>
              </>
            )}
