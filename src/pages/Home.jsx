...
import { platformMetrics, steps } from "../utils/mockData"

const FILTROS = ["Todos", "Urgentes", ...["Higiene & Cuidados Pessoais", "Alimentação", "Educação & Escolar", "Saúde & Medicamentos", "Moradia & Utensílios"]]
...
export default function Home() {
  ...
  const visiveis = campanhas.filter((c) => {
    if (filtro === "Urgentes") return c.is_urgent
    if (filtro === "Todos") return true
    return c.category === filtro
  })
    ...
    <>
      <section id="home" className="overflow-hidden bg-linear-to-r from-page via-soft to-white">
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="relative">
            <div className="absolute left-4 top-4 z-10 text-5xl text-purple-300/60 sm:left-8 sm:top-6 sm:text-7xl">♡</div>
            ...
            <img src={kaoImage} alt="Crianças em ambiente de cuidado inclusivo"
      ...
      <section id="campaigns" className="relative z-10 -mt-4 rounded-t-2xl bg-white py-12 sm:-mt-8 sm:rounded-t-4xl sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          {loading ? (
            <p className="py-12 text-center text-muted text-sm">Carregando campanhas...</p>
          ) : visiveis.length === 0 ? (
            <p className="py-12 text-center text-muted text-sm">Nenhuma campanha encontrada.</p>
          ) : (
          ...
          <div className="grid gap-4 rounded-2xl bg-gradient-to-r from-purple-700 via-purple-600 to-purple-950 p-5 text-white shadow-2xl shadow-purple-950/20 sm:gap-6 sm:rounded-3xl sm:p-8 md:grid-cols-3">
            {platformMetrics.map((metrica) => (
              <div key={metrica.label} className="flex items-center gap-4 sm:gap-5">
                ...
                <div>
                  <strong className="block text-2xl font-black sm:text-3xl">{metrica.value}</strong>
