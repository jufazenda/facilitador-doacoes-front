...
export default function InstitutionRegistration() {
  ...
  function handleSubmeter(e) {
    e.preventDefault()
    console.log("cadastro instituição:", form, arquivo, logo)
  }
        ...
        <div className="bg-white rounded-xl border border-line p-6 flex flex-col gap-5 sm:p-8 sm:gap-6">
          ...
          <form onSubmit={handleSubmeter} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-ink">Logo da instituição</label>
              <p className="text-xs text-muted">Aparecerá nos cards de campanha (JPG, PNG ou SVG)</p>
              <label className="flex items-center gap-4 cursor-pointer group">
                <div className="h-16 w-16 shrink-0 overflow-hidden rounded-full border-2 border-dashed border-line bg-soft flex items-center justify-center group-hover:border-primary transition-colors">
                  {logoPreview ? (
                ...
                <span className="text-sm text-muted group-hover:text-primary transition-colors">
                  {logo ? logo.name : "Clique para adicionar o logo"}
                </span>
